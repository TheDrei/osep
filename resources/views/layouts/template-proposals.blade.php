            ...
            <div class="tab-pane fade comments-tab" id="program_comments" role="tabpanel" aria-labelledby="program_comments_tab">
              <div class="d-block comments-list">
                <div class="text-center my-3">
                  ...
                  <div class="clearfix py-2">
                        <label class="btn btn-success float-right">
                          Upload Attachment | <i class="fas fa-caret-down"></i> 
                          <input id="program_upload_comment_attachment" type="file" hidden>
                        </label>
                      </div>
                </div>
                <ul id="program_comments_list" class="proposal-info-list proposal-comments-list">
                  ...
                  <li class="py-1 text-break">
                    ...
                    <ul id="program_comment_files_list" class="comment-files-list"></ul>
                  </div>
                  <!-- End added by Drei -->
                <hr>
              ...
              <div class="tab-content" id="project_content">
                ...
                <div class="tab-pane fade comments-tab" id="project_comments" role="tabpanel" aria-labelledby="project_comments_tab">
                  <div class="d-block comments-list">
                    <div class="text-center my-3">
                      ...
                      <div class="clearfix py-2">
                        <label class="btn btn-success float-right">
                          Upload Attachment | <i class="fas fa-caret-down"></i> 
                          <input id="project_upload_comment_attachment" type="file" hidden>
                        </label>
                      </div>
                    </div>
                 ...
                 <!-- Added by Drei -->
                    <div class="alert alert-files proposal-info-list">
                    <h4>Comment Attachments</h4>
                    <ul id="project_comment_files_list" class="comment-files-list"></ul>
                  </div>
                  <!-- End added by Drei -->
...
@section('script')
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function() {
      ...
      function forward_proposal_to_palihan_direct(proposal_id) {
        var request = $.ajax({
          url: "{{ route('proposals.forward_proposal_to_palihan_direct')}}",
          method: 'POST',
          data: {
            '_token' : '{{ csrf_token() }}',
            'proposal_id' : proposal_id,
      ...
      function forward_proposal_to_trd(forward_proposal_divisions, forward_proposal_users, lead_division, status_remarks) {
       console.log(forward_proposal_divisions, forward_proposal_users, lead_division, status_remarks);
        var request = $.ajax({
     ...
     function delete_comment_attachment(proposal_id, file_id){
        var request = $.ajax({
          url: "{{ route('proposals.delete_comment_attachment')}}",
          method: 'DELETE',
          data: {
            '_token' : '{{ csrf_token() }}',
            'proposal_id' : proposal_id,
      ...
      //Added by Drei
      function upload_comment_attachment(proposal_file_upload, proposal_id, document_type){
        var formData = new FormData();
        formData.append('proposal_id', proposal_id);
        formData.append('document_type', document_type);
        formData.append('proposal_file', $(proposal_file_upload).prop('files')[0]);
        ...
        var request = $.ajax({
          method: 'POST',
          url: "{{ route('proposals.upload_comment_attachment') }}",
          contentType: false,
          processData: false,
          data: formData
        })
            ...
            if(data['proposal_type'] == 1 || data['proposal_type'] == 3 || data['proposal_type'] == 5) {
              ...
              if(data['proposal_files'].length > 0){
                for (var i = 0; i < data['proposal_files'].length; i++) {
                  ...
                  if(data['proposal_files'][i]['dpmis_doc_id'] == null || data['proposal_files'][i]['dpmis_doc_id'] == '') {
                  $('#internal_program_files_list').append(proposal_file)
                  }
                  else $('#dpmis_program_files_list').append(proposal_file)
                } 

                if(data['proposal_comment_attachment'].length > 0){
                for (var i = 0; i < data['proposal_comment_attachment'].length; i++) {
                  // console.log(data['proposal_comment_attachment'][i]['lib_document_type_idlib_document_type'])
              
                  proposal_comment_attachment = '<li class="comment-attachment-li-'+i+' py-1 text-break"> <b>From <i>('+data['proposal_comment_attachment'][i]['proposal_code']+' (Date uploaded: '+data['proposal_comment_attachment'][i]['dpmis_date_uploaded']+') ):<i></b> <a class="text-dark proposal-file-link" href="'+data['proposal_comment_attachment'][i]['file_src']+'" target="_blank">'+data['proposal_comment_attachment'][i]['file_name']+'Date uploaded: '+data['proposal_comment_attachment'][i]['dpmis_date_uploaded']+'</a></li>&nbsp;<button class="btn-delete btn-ctr-'+i+' btn-danger">Delete</button>'

                  

                  if(data['proposal_comment_attachment'][i]['dpmis_doc_id'] == null || data['proposal_comment_attachment'][i]['dpmis_doc_id'] == '') {
                  $('#program_comment_files_list').append(proposal_comment_attachment)
                  }
                  else { $('#program_comment_files_list').append('<div class="text-center"><h3>None</h3></div>') }
                  } 
                  }

                  $('#program_comment_files_list').on('click', '.btn-delete', function(e) {
                    e.preventDefault()
                    current_proposal_modal = $('.view-proposal-modal.modal.show')
                    var attachmentIndex = $('.btn-delete').index(this);
                    $.confirm({
                      title: 'Notification!',
                      ...
                      buttons: {
                        ...
                        Yes : {
                          ...
                          action: function(){
                            var file_id = data['proposal_comment_attachment'][attachmentIndex]['idproposal_file'];  
                            var proposal_id = data['proposal_comment_attachment'][attachmentIndex]['proposal_idproposal'];     
                            delete_comment_attachment(proposal_id, file_id);
                            request.then(function(data){
                              if(data['status'] == 1){
                                $.alert({
                                  title: 'Notification!',
                                  content: 'Comment Attachment successfully deleted!',
                                  buttons: {
                                    Confirm : function(){
                                      $(this).parent().alert('close'); 
                                      $(current_proposal_modal).find('.comment-attachment-li-'+attachmentIndex+'').remove();
                                      $(current_proposal_modal).find('.btn-ctr-'+attachmentIndex+'').remove();
                                    }
                                  }
                                })
                              }
              ...
              if(data['proposal_files'].length > 0){
                ...
                for (var i = 0; i < data['proposal_files'].length; i++) {
                  ...
                  if(data['proposal_files'][i]['dpmis_doc_id'] == null || data['proposal_files'][i]['dpmis_doc_id'] == '') {
                  $('#internal_project_files_list').append(proposal_file)
                  }
                ...
                for (var i = 0; i < data['proposal_comment_attachment'].length; i++) {
                  ...
                  proposal_comment_attachment = '<li class="comment-attachment-li-'+i+' py-1 text-break"> <b>From <i>('+data['proposal_comment_attachment'][i]['proposal_code']+')<i></b> <a class="text-dark proposal-file-link" href="'+data['proposal_comment_attachment'][i]['file_src']+'" target="_blank">'+data['proposal_comment_attachment'][i]['file_name']+'</a></li>&nbsp;<button class="btn-delete btn-ctr-'+i+' btn-danger">Delete</button>'

                  var file_id = data['proposal_comment_attachment'][i]['idproposal_file'];     
                  $('#project_comment_files_list').on('click', '.btn-delete', function() {
                  
                  });

                  if(data['proposal_comment_attachment'][i]['dpmis_doc_id'] == null || data['proposal_comment_attachment'][i]['dpmis_doc_id'] == '') {
                  $('#project_comment_files_list').append(proposal_comment_attachment)
                  }
                  else { $('#project_comment_files_list').append('<div class="text-center"><h3>None</h3></div>') }
                  } 
                  }

                  $('#project_comment_files_list').on('click', '.btn-delete', function(e) {
                    e.preventDefault()
                    current_proposal_modal = $('.view-proposal-modal.modal.show')
                    var attachmentIndex = $('.btn-delete').index(this);
                    $.confirm({
      ...
      $('#proposals_table').on('click', '.forward-proposal-to-palihan-direct', function(e) {
        e.preventDefault()
         proposal_id = $(this).parents('tr').attr('data-id')
        $.confirm({
          title: 'Notification!',
          ...
          buttons: {
            Yes: function(){
              loader.open()
              var request_forward = forward_proposal_to_palihan_direct(proposal_id);
              // console.log(data)
              request_forward.then(function(data){
                loader.close();
                console.log(data);
                if(data == 1){
                  $.alert({
                    title: 'Notification',
                    content: 'Forward to Palihan (Direct) successful!',
                    buttons:{
                      Confirm: function(){
                        proposals_table.ajax.reload(null, false)
                      }
                ...
                } else show_notif('Forward to Palihan (Direct) Failed')
              })
            },
            No: function(){
      ...
      //Added by Drei - Upload Comment Function
      $('#project_upload_comment_attachment').on('change', function(e){
        proposal_file_upload = $(this)
        current_proposal_modal = $('.view-proposal-modal.modal.show')
        document_type = 22;
  
        var request_upload_file = upload_comment_attachment($(proposal_file_upload), proposal_id, document_type); 
        
        request_upload_file.then(function(data) {
                      proposal_file_upload.val('')
                     
                      if(data['status'] == 1) {
                        proposal_file = 
                        '<li class="py-1 text-break"><b>From <i>('+data['file']['proposal_code']+'):<i></b><a class="text-dark" href="'+data['file']['file_src']+'" target="_blank">'+data['file']['file_name']+' ('+data['file']['document_type']+')</a></li>'
                        
                        $(current_proposal_modal).find('.comment-files-list').append(proposal_file)
                      }
                      show_notif(data['view']);
                       })
      })
      ...
      $('#program_upload_comment_attachment').on('change', function(e){
        proposal_file_upload = $(this)
        current_proposal_modal = $('.view-proposal-modal.modal.show')
        document_type = 22;
